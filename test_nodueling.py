...
from RCQL import RCQLAgent, RecurrentQNetwork
...
for ep in range(150):
    ...
    while not term:
        ...
        action = q.squeeze(1).argmax(dim=1).item() if torch.rand(1).item() > 0.2 else torch.randint(0, 3, (1,)).item()
    ...
    if (ep + 1) % 50 == 0:
        eval_reward = sum([env.step(agent.act(torch.tensor(obs).to("cuda"), True).item())[1] for _ in range(20) for obs, _ in [env.reset()] for _ in range(15) if not env.step(0)[2]]) 
        print(f"Ep {ep+1} Eval:", eval_reward)
